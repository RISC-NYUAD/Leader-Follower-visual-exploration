    ...
    try{
        ...
        if (pResultImage->IsIncomplete() || pResultImage->GetImageStatus() != IMAGE_NO_ERROR) {
            ...
            if (frameID_ >= 0) {
                ...
                if(frameID_ > lastFrameID_ + 1) ROS_WARN_STREAM("Frames are being skipped!" << "Current: " << frameID_ << " Last: " << frameID_);
            } else {
                frameID_ = pResultImage->GetFrameID();
                //ROS_ASSERT_MSG(frameID_ == 0 ,"First frame ID was not zero! Might cause sync issues later...");
                ROS_WARN_STREAM("First frame ID was not zero! Might cause sync issues later...");
            }
