...
vector<vector<aruco::Marker>> RhombiMarkerDetections(NUM_RHOMBIS-1);

//because one rhombi is our own and there is no way I am going to detect it -- see also drone_self_rhombi variable
vector<vector<cv::Mat>> RvecTvec(NUM_RHOMBIS-1); 


const std::string drone_name="mini8";
//This is to avoid erroneous measurements were a drone claims it detected a rhombi geometry which is actually itself
int drone_self_rhombi = marker_start_ids[0]/17; 
...
bool separate_markersNE(vector<aruco::Marker> Markers){
	...
	bool found=false;
	std::vector<int> rhombi_exists(NUM_RHOMBIS-1,0);
	for(auto& marker : Markers){
		...
		if(index<NUM_RHOMBIS && index!=drone_self_rhombi){
			RhombiMarkerDetections[index-1].push_back(marker);
			found=true;
		}
	}
	return found;
