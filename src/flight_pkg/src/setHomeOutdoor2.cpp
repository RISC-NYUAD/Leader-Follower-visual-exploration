...
int main(int argc, char **argv)
{
  ...
  //set the original orientation of the gym
  GYM_OFFSET = 0;
  for (int i = 0; i < 10; i++) {
    ros::spinOnce(); 
    rate.sleep();
    GYM_OFFSET += compass_hdg;
    ROS_INFO("current heading %f", GYM_OFFSET);
  }
  GYM_OFFSET /= 10;
  ...
  tf::Quaternion q(tf::createQuaternionFromYaw(GYM_OFFSET));
  homePosMsg.q[0] = q.getW(); // w x y z
  homePosMsg.q[1] = q.getX();
  homePosMsg.q[2] = q.getY();
  homePosMsg.q[3] = q.getZ();
