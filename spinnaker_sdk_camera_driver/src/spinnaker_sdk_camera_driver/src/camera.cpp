...
ImagePtr acquisition::Camera::grab_frame() {
    ...
    try{
        ...
        if (pResultImage->IsIncomplete() || pResultImage->GetImageStatus() != IMAGE_NO_ERROR) {
            //if ( pResultImage->GetImageStatus() == IMAGE_NO_ERROR ) {
            ROS_DEBUG_STREAM("Image incomplete with image status " << pResultImage->GetImageStatus() << "!");
        } else {
		...
		if(frameID_ > lastFrameID_ + 1) ROS_DEBUG_STREAM("Frames are being skipped!" << "Current: " << frameID_ << " Last: " << frameID_);
            } else {
                frameID_ = pResultImage->GetFrameID();
	...
	ROS_WARN_STREAM(e.what()<<" GET_NEXT_IMAGE_TIMEOUT_ timed out \n");
    }
    return pResultImage;
}
...
Mat acquisition::Camera::grab_mat_frame() {
    ...
    try{
        ...
        if(!pResultImage->IsIncomplete() && pResultImage->GetImageStatus() == IMAGE_NO_ERROR)    img= convert_to_mat(pResultImage);
...
Mat acquisition::Camera::convert_to_mat(ImagePtr pImage) {
    ...
    //pImage->Release(); --> I do this in grab_mat_frame() now
...
bool acquisition::Camera::getGigEType(){
	...
	if (ptrDeviceType->GetIntValue() == DeviceType_GigEVision) return true;
    }
    return false;
}
