    ...
    for (int i = numCameras_-1 ; i >=0 ; i--) {
        ...
        try {
            ...
            if (!soft) {
      		...
      		//NE disable heartbeat for my GIGE camera
		if(cams[i].getGigEType()){ 
			ROS_INFO("Working with GigE camera. Disabling heartbeat for more bandwidth");
			cams[i].setBoolValue("GevGVCPHeartbeatDisable",true);
		}
